...
const Stake = () => {
	...
	const res =await axios.post(`http://localhost:8888/user/stake/${id}`,{stake:Number(stakevalue.stakeInput)},{headers : {Authorization: `Bearer ${token}`}});
	console.log(res)
	getData()
	}
			...
			<div className="h-800 sm:h-[500px] lg:h-[700px] bg-[rgb(0,0,0)] ">
				<div className="border- lg:w-[80%] h-full sm:h-150 m-auto flex flex-col items-center justify-around ">
					<div className="border- h-40  w-[99%] grid grid-cols-1 sm:grid-cols-3 gap-10 place-items-center">
						<div className="bg-[rgb(22,22,26)]   shadow-[rgba(0,0,0,0.1)_0px_4px_6px_-1px,rgba(0,0,0,0.06)_0px_2px_4px_-1px] border-[rgba(245,158,11,0.3)] h-[160px] w-[95%] xl:h-full xl:w-80 rounded-2xl  p-5">
							<div className="flex flex-col gap-4 text-center sm:text-left">
								...
								<div className="flex gap-2 justify-center sm:justify-start">
									<p className="text-white">{data.wallet}</p>
						...
						<div className="bg-[rgb(22,22,26)]   shadow-[rgba(0,0,0,0.1)_0px_4px_6px_-1px,rgba(0,0,0,0.06)_0px_2px_4px_-1px] border-[rgba(245,158,11,0.3)] h-[160px] w-[95%] xl:h-full xl:w-80 rounded-2xl p-5">
							<div className="flex flex-col gap-4 text-center sm:text-left">
								...
								<div className="flex gap-2 justify-center sm:justify-start">
									<p className="text-white">{data.stake}</p>
									<img src={Hcoin} className="w-4" alt="" />
								</div>
					...
					<div className="border- h-60 w-[99%] grid sm:grid-cols-2 ">
						<div className="bg-[rgb(22,22,26)]   shadow-[rgba(0,0,0,0.1)_0px_4px_6px_-1px,rgba(0,0,0,0.06)_0px_2px_4px_-1px] border-[rgba(245,158,11,0.3)] h-[220px] w-[95%] xl:h-full  rounded-2xl  p-8 ">
							<div className="flex flex-col gap-4 text-center sm:text-left">
								<div className="flex  gap-2 justify-center sm:justify-start">
								...
								<div
									onClick={SendStake}
									className="text-white rounded-[6px] flex h-12 justify-center items-center gap-2 text-[20px] bg-amber-500 hover:shadow-[0_4px_12px_0px_rgba(217,119,6,0.4)] hover:scale-y-110 duration-200 transition-all"
								>
									<p>+</p>
									<p>Stake Now</p>
								</div>
						...
						<div className="bg-[rgb(22,22,26)]   shadow-[rgba(0,0,0,0.1)_0px_4px_6px_-1px,rgba(0,0,0,0.06)_0px_2px_4px_-1px] border-[rgba(245,158,11,0.3)] h-[220px] w-[95%] xl:h-full  rounded-2xl  p-8"></div>
					</div>
				</div>
			</div>
		</div>
